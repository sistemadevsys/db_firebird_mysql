...
try:
    ...
    cursor_fire.execute("""SELECT REFERENCIAL, REF_FUN, DESCRICAO,
                           RESPONSAVEL, DT_AGENDA, VALOR, EQUIPAMENTO
                           FROM SER_SERVICOS""")
    t_ser = cursor_fire.fetchall()

    # Ordem Servicos
    cursor_mysql.execute("""SELECT referencial, ref_fun2, descricao,
                    dt_agenda, valor, usuario_os_id FROM core_ordem_servico
                    """)
    t_os = cursor_mysql.fetchall()

    sql_fun = ("""SELECT referencial, nome
    ...
    for rfire, rfun, desc, respo, dt_a, vl, eq in t_ser:
        for rfos, rfun_os, descri, dt, vl_os, user_id in t_os:
            for rfu, nome in t_mysql_fun:  # Funcionario
                if rfu == rfun and descri is None:  # user e OS
                    ...
                    sql_i = """INSERT INTO core_ordem_servico(
                        ...
                        dt_agenda, dt_entrada, dt_atualizada, valor,
                        equipamento, usuario_os_id)
                        VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                    val = (
                        rfire, rfun, desc, respo, dt_a, dt_, dt_,
                        vl, eq, user_id
                        )
